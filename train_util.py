...
def infogan_generator(inputs, categorical_dim, weight_decay=2.5e-5):
    ...
    with slim.arg_scope(
        ...
        net = tf.reshape(net, [-1, 8, 8, 128]) # 8x8x128
        net = layers.conv2d_transpose(net, 64, [4, 4], stride=2) # 16x16x64
        net = layers.conv2d_transpose(net, 32, [4, 4], stride=2) # 32x32x32
        # Make sure that generator output is in the same range as `inputs`
        # ie [-1, 1].
        net = layers.conv2d(net, 3, [4,4], normalizer_fn=None, activation_fn=tf.sigmoid) # 32x32x3
